<template>
  ...
  const handleEventClick = (info) => {
    ...
    Swal.fire({
      ...
      html: `
        ...
        <p><strong>Date de Début :</strong> ${new Date(appointment.date_debut).toLocaleString('fr-FR')}</p>
        <p><strong>Date de Fin :</strong> ${new Date(appointment.date_fin).toLocaleString('fr-FR')}</p>
      `,
      icon: 'info',
  ...
  const calendarOptions = ref({
    ...
    buttonText: { // Texte des boutons en français
      today: "Aujourd'hui",
      month: 'Mois',
      week: 'Semaine',
      day: 'Jour',
